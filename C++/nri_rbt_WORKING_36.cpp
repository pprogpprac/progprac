...
public:
	...
	void print_tree_level_order(node *root)
	{
		...
		while(!Q.empty())
		{
			...
			std::cout << "print_tree_level_order(): current->id = " << current->id << "\n";
			if (current != nullptr && current->parent != nullptr)
				currpar = current->parent;
	...
	void remove_node(int id)
	{
		...
		//Case 1 of 5: no node // WORKS ! VERIFIED !
		if (nullptr == temp)
		{
			std::cout << "remove_node(): Case 1 of 5: no node.\n";
		...
		{
			std::cout << "remove_node(): Case 2 of 5: root.\n";
			delete_tree();
			//delete this;
		}

		// Case 3 of 5: leaf node // WORKS ! VERIFIED !
		else if (temp->lt == nullptr && temp->gt == nullptr)
		{
			// leaf
			std::cout << "remove_node(): Case 3 of 5: leaf node.\n";
			delete_node(temp);
		}
		...
		{
			std::cout << "remove_node(): Case 4 of 5: Only Less than child exists.\n";
		...
		{
			std::cout << "remove_node(): Case 4 of 5: Only Greater than child exists.\n";
		...
		{
			std::cout << "remove_node(): Case 5 of 5: Has two children.\n";
			delete_node(temp);
		}
	}
	...
	{
		std::cout << "delete_tree(): DELETING node " << (temp->id) << "\n";
		if (temp->id == 0)
		{
			root = nullptr;
			return;
	...
	{
		root = nullptr;
		std::cout << "~nri_tree(): Could come here\n";
		//--------------------------------------------------------------------
		...
		{
			if (root != nullptr)
				std::cout << "Something worng\n";
			print_tree();
			visualize_tree();
			root = nullptr;
			std::cout << "~nri_tree(): delete_tree() was already called.\n";
			print_tree();
		    visualize_tree();
		}
