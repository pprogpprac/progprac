...
- BUG ALERT ! Search for: "BUG ALERT 1 ! " - ACTION REQUIRED: Account for similar bugs in restructuring()'s other 3 cases as well.
...
private:
	...
	struct node
	{
	    node():color(true), id(-17), space(0), level(-17), parent(nullptr), lt(nullptr), gt(nullptr){}
...
public:
	...
	void print_tree(struct node *temp, int tlev)
	{
	    ...
	    if (temp != nullptr)
	    {
			...
			//std::cout << "DEBUG: numDigits = " << numDigits(temp->id) << "\n";
			id_spaces.push_back(numDigits(temp->id));
	...
	// Obtain the node id which takes the most spaces for visualization in the entire tree.
	// For example, if following are the node ids: 12, 6, 17, 113, 4427, 0.0001, then the function
	// get_most_spaces() will return 6 since 0.0001 (being the most space consuming of all)
	// consumes 6 spaces.
	// If node IDs are only "int"s, then get_most_spaces() will return 4 since 4427 (being the most
	// space consuming of all) consumes 4 spaces.
	int get_most_spaces()
	{
		int max = 0;
		for (int i = 0; i < id_spaces.size(); i++)
		{
			//std::cout << "id_spaces = " << id_spaces[i] << "\n";
			if (i > 0)
			{
				if(id_spaces[i] > max)
					max = id_spaces[i];
	...
	/*
	void assign_node_levels()
	{
		assign_node_levels(root, -1);
	}

	void assign_node_levels(struct node *temp, int tlev)
	{
	    if (temp != nullptr)
	    {
			++tlev;
	...
	*/
	...
	/*
	void assign_node_spaces()
	{
		assign_node_spaces(root);
	}

	void assign_node_spaces(struct node *temp)
	{
		const int const_space = get_most_spaces();
		if (temp != nullptr)
		{
			if(temp->parent == nullptr) // means Root
			{
				temp->space = get_max_tree_level() * const_space;
			}
			else
			{
				if (has_sibling == true)
	...
	*/
	...
	void level_order(node *root)
	{
		...
		//std::cout << "max tee level = " << get_max_tree_level() << "\n";
		std::cout << "\n------------------------------------------------>\n";
		std::cout << "LEVEL |" << "-----TREE-------------------------------->\n";
		std::cout << "------------------------------------------------>\n";
		...
		int max_level = get_max_tree_level();
		...
		while(!Q.empty())
		{
			...
			//std::cout << "\n\n" << current->id << " printed the sibling? " << has_sibling <<"\n\n";
			...
			if (current->color == 1)
			{
				//std::cout << "Has sibling?" << has_sibling <<"\n";
				//std::cout << red << "  (" << current->id <<", "<< "RED" << ")  ";
				for (int k = 0; k < (max_level - current->level); k++)
				{
				...
				std::cout << red << "(" << current->id << ")          ";
			}
			else
			{
				//std::cout << "Has sibling?" << has_sibling <<"\n";
				//std::cout << black << "  (" << current->id <<", "<< "BLACK" << ")  ";
				for (int k = 0; k < (max_level - current->level); k++)
				{
					if (has_sibling == false)
					{
