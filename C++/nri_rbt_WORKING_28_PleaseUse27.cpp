...
public:
	...
	void post_order(node* p, int indent = 0)
	{
		if(p != nullptr)
		{
	        if(p->gt)
			{
	            post_order(p->gt, indent + 4);
	        }
	        if (indent)
			{
	...
	bool check_rules_assign_color(struct node *temp)
	{
		if (temp->parent != nullptr) // which means its NOT root.
		{
			...
			{
				std::cout << "check_rules_assign_color(): parent color is black/null, so returning true.\n";
				std::cout << "check_rules_assign_color(): temp->id = " << temp->id << "\n";
				return true;
			}
		...
		else
		{
			...
			{
				show_details_of_node(temp->id);
				std::cout << "***check_rules_assign_color(): parent exists ! temp->id = " << temp->id << " 's->parent == " << temp->parent << "\n";
			}
			else
			{
				show_details_of_node(temp->id);
				std::cout << "|||";
				std::cout << "***check_rules_assign_color(): parent doesn't exist ! temp->id = " << temp->id << " 's->parent == nullptr\n";
			}
		}
	...
	void show_details_of_node(int id)
	{
		node *temp = search_tree(id);
		std::cout << "\nYYYY\n";
		//check_rules_assign_color(temp);
		std::cout << "\nXXXX\n";
		if (temp == nullptr)
		{
			std::cout << "\nshow_details_of_node(): There is no such node !\n";
		}
		else
		{
			if (temp->parent == nullptr)
			{
				std::cout << "\nshow_details_of_node(): " << temp->id << " is the root/ Does not have parent.\n";
			}
			else
			{
				node *par = temp->parent;
				std::cout << "\nshow_details_of_node(): " << temp->id << " 's Parent is = " << par->id << "\n";
				if (temp->lt == nullptr)
				{
					std::cout << "show_details_of_node(): " << temp->id << " has no lt child.\n";
				}
				else
				{
					node *l = temp->lt;
					std::cout << "show_details_of_node(): " << temp->id << " 's less than/left child is = " << l->id << "\n";
				}
				if (temp->gt == nullptr)
				{
					std::cout << "show_details_of_node(): " << temp->id << " has no gt child.\n";
				}
				else
				{
					node *g = temp->gt;
					std::cout << "show_details_of_node(): " << temp->id << " 's greater than/right child is = " << g->id << "\n";
				}
			}
		}
	}
