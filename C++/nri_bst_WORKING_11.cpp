...
public:
	...
	void print_bst(struct node *temp, int tlev)
	{
	    ...
	    if (temp != nullptr)
	    {
			...
			temp->level = tlev;
			if (foo == nullptr)
				std::cout<< "Level: " << tlev << " || Node: " << temp->data << " || Parent node: NULL\n";
			else
				std::cout<< "Level: " << tlev << " || Node: " << temp->data << " || Parent node: " << foo->data << "\n";
	    }
		//std::cout << "DEBUG: root->data = " << root->data << "\n";
	}
	...
	void delete_node(int id)
	{
		...
		*/
		else if (temp->lt != nullptr && temp->gt != nullptr)
		{
			...
			/*struct node* foo_lt_child = foo->lt;
			struct node* foo_gt_child = foo->gt;

			foo_lt_child->parent = foo;
			foo_gt_child->parent = foo;*/
			...
			/*struct node *foos_parent = foo->parent;

			if (foos_parent->lt == foo)
				foos_parent->lt = temp->parent;
			else if (foos_parent->gt == temp)
				foos_parent->gt = temp->parent;*/

			/*temp->gt = foo->gt; // Now, 14's gt points to 17
			temp->lt = foo->lt; // Now, 14's lt points to 13
			temp->parent = foo->parent; // Even the parent.*/

			std::cout << "temp data SHOULD be 14 = " << temp->data << "\n";
