...
public:
	...
	void remove_node(int id)
	{
		...
		else if (temp->lt == nullptr && temp->gt == nullptr && temp->parent == nullptr && temp->color == 0)
		{
			...
			struct node *temppar = temp->parent;
			if(temppar->lt == temp)
				temppar->lt = nullptr;
			...
			if(temppar->color == 0) // Only if the leaf node is black then, there is a shortage of black.
				check_rules_assign_color(temppar);
			*/
		}
		...
		else if (temp->lt == nullptr && temp->gt == nullptr && temp->parent != nullptr && temp->color == 1)
		{
			...
			else if (temp->color == 1 || p->color == 1)
			{
