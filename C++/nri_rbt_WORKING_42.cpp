...
private:
	...
	bool was_root_freed = false; // Manipulated by delete_tree() and ~nri_tree().
...
public:
	...
	void remove_node(int id)
	{
		...
		{
			std::cout << "remove_node(): Case 7: 2CNB.\n";
			root = temp->gt; //U can choose either lt or gt to replace root. I choose gt.
			root->color = false;
			root->parent = nullptr;
			root->lt = nullptr;
			root->gt = nullptr;
		...
		{
			std::cout << "remove_node(): Case 8: 2CYR.\n";
			...
			std::cout << "remove_node(): Case 8: Big ID = " << big->id << "\n";
			std::cout << "remove_node(): Case 8: Big Level = " << big->level << "\n";
			std::cout << "remove_node(): Case 8: Small ID = " << small->id << "\n";
			std::cout << "remove_node(): Case 8: Small Level = " << small->level << "\n";

			// Step 3 of 3: Now the decision.
			if (big->level > small->level)
			{
				std::cout << "remove_node(): Case 8: Decided to go with biggest from lt subtree.\n";
			...
			{
				std::cout << "remove_node(): Case 8: Decided to go with smallest from gt subtree.\n";
			...
			*/
			...
			std::cout << "remove_node(): Case 8: 2CYR: Node to be replaced = " << foo->id << "\n";
			std::cout << "remove_node(): Case 8: 2CYR: Replacement candidate node = " << temp->id << "\n";
	...
	void delete_node_with_two_children(struct node *temp)
	{
		struct node *temppar = nullptr, *temps_only_child = nullptr;
		// Take a backup of the node-to-be-deleted first.
		struct node * foo = nullptr;

		if (temp->lt == nullptr && temp->gt == nullptr) // Means only one node esits
	...
	// delete_tree()'s flag was_root_freed is free of "same-object" dependence in main(). However, other boolean flags aren't, like were_levels_assigned.
	void delete_tree()
	{
